...
def processar_xmls(envio_file, retorno_file):
    ...
    for elemento in root_env.findall('.//*', ns):
        tag_full_name = elemento.tag
        tag_name = tag_full_name.split('}')[-1]
        ...
        # REGRA SADT: Identifica se é guia SADT e busca data/hora do envio
        if tag_name == 'guiaSP-SADT':
            dt_inicio = elemento.find('.//ans:dataExecucao', ns) or elemento.find('.//ans:dataRealizacao', ns)
            # No SADT a hora costuma estar vinculada ao faturamento ou atendimento
